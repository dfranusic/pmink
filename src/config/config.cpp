...
/*
std::string* config::CfgNotification::get_user_id(){
    return &user_id;
}

std::string* config::CfgNotification::get_user_type(){
    return &user_type;
}



void config::CfgNotification::set_user_id(const char* _id){
    user_id.clear();
    user_id.append(_id);
}

void config::CfgNotification::set_user_type(const char* _type){
    user_type.clear();
...
bool config::ConfigItemSort::operator()(ConfigItem* i, ConfigItem* j){
    //std::cout << "!!!SORT 1: " << i->name << "<" << j->name << std::endl;
    if(i->sort_node == NULL || j->sort_node == NULL){
	//std::cout << "!!!SORT 2: " << i->sort_node << "<" << j->sort_node << std::endl;
	return strcmp(i->name.c_str(), j->name.c_str()) < 0;
    }
    ConfigItem* s1 = (*i)(i->sort_node->name.c_str());
    ConfigItem* s2 = (*j)(j->sort_node->name.c_str());
    if(s1 == NULL || s2 == NULL) return strcmp(i->name.c_str(), j->name.c_str()) < 0;
    int is1 = s1->to_int();
    int is2 = s2->to_int();
    //std::cout << "!!!SORT: " << is1 << "<" << is2 << std::endl;
    return is1 < is2;
}
...
int config::ConfigItem::find(ConfigItem* item){
    for(unsigned int i = 0; i<children.size(); i++) if(children[i] == item) return i;
    return -1;
}
...
void config::ConfigItem::set_on_change_handler(CfgNtfCallback* _on_change, bool recursive){
    ...
    if(recursive) {
	for(unsigned int i = 0; i<children.size(); i++) children[i]->set_on_change_handler(_on_change, recursive);
    }
}
...
config::ConfigItem* config::ConfigItem::operator ()(const char* _name, bool create, ConfigNodeType last_nt, bool _set_new_flag){
    // check input
    ...
    int res_size = 0;
    // tokenize
    pmink_utils::tokenize(&line, tokens, 50, &res_size, false);
	...
	for(unsigned int j = 0; j<cur_item->children.size(); j++) if(!cur_item->children[j]->is_template){
	    // token matched
    ...
    if(ptrn != NULL){
		...
		if(ptrn->pattern.size() > 9) {
		    ...
		    if(ptrn->pattern[7] == '[' && ptrn->pattern[ptrn->pattern.size() - 1] == ']'){
			    ...
			    //std::cout << "SPECIAL PTRN: " << tmp_str << std::endl;
				...
				tmp_str.erase(0, 1);
				// get node list
				tmp_cfg = (*cfg->get_definition_root())(tmp_str.c_str());
				...
				for(unsigned int i = 0; i<tmp_cfg->children.size(); i++) if(!tmp_cfg->children[i]->is_template){
				    new_node = new config::ConfigItem();
				    new_node->name = "<" + tmp_cfg->children[i]->name + ">";
				...
				// free mem later
				for(unsigned int i = 0; i<result->children.size(); i++) tmp_node_lst->children.push_back(result->children[i]);
...
void config::ConfigItemRBR::special_ac(void** args, int argc){
	...
	for(int i = 0; i<n; i++){
		...
		pmink_utils::tokenize(&line, tokens, 10, &res_size, false);
		ifs.close();
...
bool config::UserId::operator != (const UserId& right){
    //std::cout << "comparing A = '" << user_id << "' B = '" << right.user_id << "' = " << memcmp(this, &right, sizeof(*this)) << std::endl;
    //return memcmp(this, &right, sizeof(right)) != 0;
    return memcmp(this->user_id, right.user_id, sizeof(right.user_id)) != 0 || memcmp(this->user_type, right.user_type, sizeof(right.user_type)) != 0;
}
...
// User Id compare
bool config::UserIdCompare::operator ()(const UserId& x, const UserId& y){
    //return memcmp(&x, &y, sizeof(x)) < 0;
    return memcmp(x.user_id, y.user_id, sizeof(x.user_id)) < 0;// || memcmp(x.user_type, y.user_type, sizeof(x.user_id)) < 0;
...
config::Config::~Config(){
	// remove cmd tree pointers to definition
	//cmd_tree->children[0]->children.clear();
	//cmd_tree->children[1]->children.clear();
...
void config::Config::flatten(ConfigItem* tree, ConfigItem* output){
	...
	if(!tree->is_template){
	    ...
	    for(unsigned int i = 0; i<tree->children.size(); i++) flatten(tree->children[i], output);
	}
...
void config::Config::print_config_tree(ConfigItem* tree, int depth, bool ncurses){
	...
	if(ncurses){
	    printw("%s [type=%s, value=%s, nodetype=%d, nodestate=%d, is_template=%d, is_new=%d, desc=%s]", tree->name.c_str(),
													    tree->type.c_str(),
													    tree->value.c_str(),
													    tree->node_type,
													    tree->node_state,
...
int config::Config::get_commands_lc(ConfigItem* _definition){
	...
	for(unsigned int i = 0; i<_definition->children.size(); i++){
	    ...
	    // config item
	    }else if(tmp_node->node_type == CONFIG_NT_ITEM && (tmp_node->new_value != "" || tmp_node->node_state == CONFIG_NS_DELETED)){
		    ++res;
...
void config::Config::show_commands(ConfigItem* _definition, int depth, std::stringstream* out_stream){
	...
	for(unsigned int i = 0; i<_definition->children.size(); i++){
		...
		*out_stream << tmp_cmd;
		...
		// print cmd
		if(tmp_node->node_state != CONFIG_NS_DELETED){
			*out_stream << tmp_str << tmp_node->name << " \"" << tmp_node->new_value << "\"" << std::endl;
			//wprintw(win, "%s%s \"%s\"\n", tmp_str.c_str(), tmp_node->name.c_str(), tmp_node->new_value.c_str());
...
void config::Config::show_commands(ConfigItem* _definition, int depth, bool ncurses){
	...
	for(unsigned int i = 0; i<_definition->children.size(); i++){
		...
		if(tmp_node->node_state == CONFIG_NS_DELETED){
		    ...
		    tmp_node = _definition->children[i];
		    // print cmd
		    if(ncurses) printw("delete %s%s\n", tmp_str.c_str(), tmp_node->name.c_str()); else std::cout << "delete " << tmp_str << tmp_node << std::endl;
		...
		// print cmd
		if(tmp_node->node_state != CONFIG_NS_DELETED){
			if(ncurses) printw("%s%s \"%s\"\n", tmp_str.c_str(), tmp_node->name.c_str(), tmp_node->new_value.c_str());
			else std::cout << tmp_str << tmp_node->name << " \"" << tmp_node->new_value << "\"" << std::endl;
...
void config::Config::show_commands(ConfigItem* _definition,  unsigned char* result, int* result_size, int depth){
...
void config::Config::show_config(ConfigItem* _contents, int depth, int* result_size, bool no_output, std::ofstream* out_stream, bool no_uncm, std::string* desc){
    if(_contents != NULL){
	ConfigItem* tmp_node = NULL;
	int tmp_size = 0;
	int extra = 0;
	...
	for(unsigned int i = 0; i<_contents->children.size(); i++) if(!_contents->children[i]->is_template && _contents->children[i]->node_type == CONFIG_NT_ITEM){
	    tmp_node = _contents->children[i];
	    if(tmp_node->name.size() + (tmp_node->node_state != CONFIG_NS_READY ? 1 : 0) > max_length) max_length = tmp_node->name.size() + (tmp_node->node_state != CONFIG_NS_READY ? 1 : 0);
	}
	    ...
	    // config item
		    ...
		    if(!no_output){
			    ...
			    *out_stream << "\"" << *tmp_val << "\"\n";
			    ...
			    //out_stream->write(tmp_str.c_str(), tmp_str.size());
			    //out_stream->write("\t\"", 2);
			    //out_stream->write(tmp_val->c_str(), tmp_val->size());
			    //out_stream->write("\"\n", 2);
...
void config::Config::show_config(ConfigItem* _contents, int depth, int* result_size, bool no_output, std::stringstream* out_stream){
    if(_contents != NULL){
	ConfigItem* tmp_node = NULL;
	int tmp_size = 0;
	int extra = 0;
	    ...
	    // config item
		    ...
		    if(!no_output){
			    ...
			    *out_stream << "\"" << tmp_node->new_value << "\"" << std::endl;
			    ...
			    //*out_stream << tmp_str << "\t" << "\"" << tmp_node->new_value << "\"" << std::endl;
...
void config::Config::show_config(ConfigItem* _contents, int depth, int* result_size, bool no_output, WINDOW* win){
    if(_contents != NULL){
	ConfigItem* tmp_node = NULL;
	int tmp_size = 0;
	int extra = 0;
...
void config::Config::show_config(ConfigItem* _contents, int depth, int* result_size, bool ncurses, bool no_output){
    if(_contents != NULL){
	ConfigItem* tmp_node = NULL;
	int tmp_size = 0;
	int extra = 0;
		...
		while(pch != NULL){
			...
			while ((ent = readdir (dir)) != NULL) {
			    if(strcmp(ent->d_name, "..") != 0){
				    if(tmp_path.compare(0, tmp_path.size(), ent->d_name, 0, tmp_path.size()) == 0){
					tmp_item = new ConfigItem();
			...
			// more results
				...
				for(unsigned int j = 0; j<result->children.size(); j++){
					if(max_match == result->children[j]->name.size()){
					    ...
					    // free mem
					    for(unsigned int i = 0; i<result->children.size(); i++) if(i != j) delete result->children[i];
					    // clear
					    result->children.clear();
					    // add saved match
					    result->children.push_back(tmp_item);
...
void config::Config::search_definition(ConfigItem* def, int current_level, int target_level, std::string* target, ConfigItem* result){
		...
		for(int i = 0; i<n; i++){
			...
			if(!tmp_ci->is_template){
				...
				// right level
				    ...
				    // compare
				    if(tmp_ci->name.compare(0, other_str.length(), other_str) == 0) result->children.push_back(tmp_ci);
				}
...
void config::Config::print_cfg_def(bool show_val, bool show_desc, ConfigItem* def, int level, int max_levels, WINDOW* win){
	if(def != NULL){
		...
		unsigned int max_length = 0;
		// calculate padding for item names
		for(unsigned int i = 0; i<n; i++) if(def->children[i]->name.size() > max_length) max_length = def->children[i]->name.size() + (def->children[i]->node_state != CONFIG_NS_READY ? 1 : 0);
		++max_length;
		// find max value length
		unsigned int max_val_length = 0;
		// calculate padding for item values
		if(show_val){
			for(unsigned int i = 0; i<n; i++) if(def->children[i]->new_value.size() > max_val_length) max_val_length = def->children[i]->new_value.size();
			++max_val_length;
		}
		// tmp strings
		std::string tmp_name;
		...
		for(unsigned int i = 0; i<n; i++){
			...
			// next level
			if((level + 1) < max_levels) print_cfg_def(show_val, show_desc, def->children[i], level + 1, max_levels, win);
		}
...
void config::Config::print_cfg_def(bool show_val, bool show_desc, ConfigItem* def, int level, int max_levels){
	if(def != NULL){
		std::string* tmp_val = NULL;
		// child count
		unsigned int n = def->children.size();
		...
		for(unsigned int i = 0; i<n; i++){
			...
			// next level
			if((level + 1) < max_levels) print_cfg_def(show_val, show_desc, def->children[i], level + 1, max_levels);
		}
...
void config::Config::copy_nodes(ConfigItem* source, ConfigItem* dest, ConfigNodeState new_state){
    if(source != NULL && dest != NULL){
		...
		tmp_new_node->node_state = (new_state == CONFIG_NS_UNKNOWN ? source->children[i]->node_state : new_state);
...
void config::Config::auto_complete(	ConfigModeType* mode,
	...
	if(def != NULL && line != NULL){
		...
		*result_size = 0;
		...
		if(line_size > 0){
			// index 0 is not used since it represents mode information(get/set) already contained in mode argument
			for(int i = 0; i<line_size; i++){
				// curent line token
				tmp_str = line[i];
				//std::cout << "Token: " << tmp_str << " - Def: " << def->name << std::endl;
				...
				if(*last_found != NULL && (*mode == CONFIG_MT_SET || *mode == CONFIG_MT_CMD)){
					...
					if((*last_found)->node_type == CONFIG_NT_ITEM || (*last_found)->node_type == CONFIG_NT_PARAM){
						// set fsys ac flag
						special_ac = false;
				...
				if(!param_found) {
					...
					if(def->children[0]->is_template){
					    ...
					    if((tmp_str[0] == '!' || result->children.size() == 0) && *mode == CONFIG_MT_SET){
				...
				if(result->children.size() == 1 && !param_found) {
					...
					*last_found = result->children[0];
					...
					if(*last_found != NULL && *mode == CONFIG_MT_UNKNOWN){
					    if((*last_found)->node_type == CONFIG_NT_CMD || (*last_found)->node_type == CONFIG_NT_BLOCK){
						...
						/*
						else if((*last_found)->name == "configuration") *mode = CONFIG_MT_CMD;
						else if((*last_found)->name == "commands") *mode = CONFIG_MT_CMD;
						else if((*last_found)->name == "top") *mode = CONFIG_MT_CMD;
						else if((*last_found)->name == "up") *mode = CONFIG_MT_CMD;
						else if((*last_found)->name == "save") *mode = CONFIG_MT_CMD;
						*/
					    }
					...
					if(result->children[0]->node_type == CONFIG_NT_BLOCK || result->children[0]->node_type == CONFIG_NT_CMD){
						// go deeper
						...
						*last_found = def;
						...
						if(ac_mode == CONFIG_ACM_ENTER){
							...
							if(*mode == CONFIG_MT_SET && i == (line_size - 1) && (*last_found)->node_state == CONFIG_NS_DELETED){
						...
						if(def->node_type == CONFIG_NT_CMD){
						    for(unsigned int i = 0; i<def->children.size(); i++) if(def->children[i]->node_type == CONFIG_NT_PARAM && !pretend) def->children[i]->new_value = "";
						}
					...
					// other
						...
						if(result->children.size() > 0){
							...
							if(line[i].size() == max_match && *last_found != NULL){
								...
								(*result_size)++;
								result->children.clear();
								// if param, include in result, useful for auto completion (display param=value list)
								if((*last_found)->node_type == CONFIG_NT_ITEM) result->children.push_back(*last_found);
...
void config::Config::replace_prepare(ConfigItem* _definition){
	...
	while(i < _definition->children.size()){
		...
		if(tmp_node->parent->children.size() > 1){
		    ...
		    for(unsigned int j = 1; j<tmp_node->parent->children.size(); j++) tmp_node->parent->children[j]->node_state = CONFIG_NS_DELETED;
...
int config::Config::reset(ConfigItem* _definition){
	...
	for(unsigned int i = 0; i<_definition->children.size(); i++){
		...
		if(tmp_node->parent->children.size() > 1){
		    // deallocate
		    for(unsigned int j = 1; j<tmp_node->parent->children.size(); j++) delete tmp_node->parent->children[j];
		    // remove
		    tmp_node->parent->children.erase(tmp_node->parent->children.begin() + 1, tmp_node->parent->children.end());
		}
	    }else{
		    tmp_node->value = "";
		    tmp_node->new_value = "";
		...
		if(!_definition->children[j]->is_template){
			if(tmp_node->name == _definition->children[j]->name){
				...
				if(tmp_node->node_type == CONFIG_NT_ITEM){
					...
					if(tmp_ptrn != NULL){
					    // validate pattern
					    res = pattern_valid(&tmp_node->value, &_definition->children[j]->type, tmp_node);
					    if(!res) return false;
					}
	...
	for(unsigned int i = 0; i<_contents->children.size(); i++){
			...
			if(tmp_node->name == _definition->children[j]->name){
				...
				if(tmp_node->node_type == CONFIG_NT_ITEM){
					...
					if(tmp_ptrn != NULL){
					    ...
					    // validate pattern
					    res = pattern_valid(&tmp_node->value, &_definition->children[j]->type, tmp_node);
					    //std::cout << "PATTERN: " << tmp_ptrn->pattern << " - " <<  res << std::endl;
					    if(res == 0) return 1;
					}
					// if flag is set, do not update NS_READY value (important for discard when loading cfg files)
		...
		// template definition
			...
			if(!new_node->is_empty){
			    // sort
			    ConfigItemSort cfg_sort;
			    std::sort(new_node->parent->children.begin() + 1, new_node->parent->children.end(), cfg_sort);
			}
		    }
		    // set as found
		    found = true;
...
config::CfgNotification* config::Config::remove_notification(CfgNotification* cfg_ntf){
    for(unsigned int i = 0; i<notifications.size(); i++) if(notifications[i] == cfg_ntf){
...
config::CfgNotification* config::Config::get_notification(std::string* cfg_path){
    for(unsigned int i = 0; i<notifications.size(); i++) if(*notifications[i]->get_cfg_path() == *cfg_path){
...
config::CFGPattern* config::Config::get_pattern(std::string* type){
	for(unsigned int i = 0; i<patterns.size(); i++) if(patterns[i]->name == *type) return patterns[i];
	return NULL;
}
		...
		if(ptrn->pattern.substr(0, 7) == ":pmcfg:"){
			...
			if(ptrn->pattern[7] == '[' && ptrn->pattern[ptrn->pattern.size() - 1] == ']'){
				...
				// relative
				    ...
				    // get node list
				    if(cfg_node != NULL){
					if(cfg_node->parent != NULL) tmp_cfg = (*cfg_node->parent)(tmp_str.c_str());
				...
				if(tmp_cfg != NULL){
				    for(unsigned int i = 0; i<tmp_cfg->children.size(); i++) if(!tmp_cfg->children[i]->is_template){
					//std::cout << "cfg item: " << tmp_cfg->children[i]->name << ", value: " << *value << std::endl;
					// try to match
					if(*value == tmp_cfg->children[i]->name) return true;
				    }
		    ...
		    //std::cout << "AAAAAAAAA: " << ptrn->pattern << std::endl;
