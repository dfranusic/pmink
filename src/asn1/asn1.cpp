...
// Session id
asn1::SessionId::SessionId(){
    session_id = 0;
}

// if session_id is zero, set all elements' session_id to zero and generate next session_id
// zero session_id is encountered when all available sessions have been used (highly unlikely) and reset is needed
// to avoid session data overlapping
...
uint64_t asn1::SessionId::get_current_id(){
    return session_id;
}
...
asn1::ASN1Pool::~ASN1Pool(){
	...
	// children and tlv have to be cleared or double free corruption can occur
	// every asn1node frees its own children but in case of pooling
	// this has to be avoided since all individual child objects are disposed of
	// separately
...
int asn1::ASN1Pool::get_tlv_count(){
	return tlv_count;
}
...
/*
void asn1::ASN1Node::change_linked_data(uint64_t _session_id, unsigned char* _data, unsigned int _data_length, bool _shallow){
	if(linked_node != NULL){
...
// when unlinking CHOICE selection, do not unlink children, unlink the CHOICE node itself
// and then set the new selection
void asn1::ASN1Node::unlink(uint64_t _session_id){
	//linked_node = this;
	if(linked_node == NULL) return;
	if(linked_node->tlv != NULL){
	    // do not unlink unused node
	    // if tag_value_size is less then 0, node has not been included in calculations, already unlinked
	    if(linked_node->tlv->tag_value_size > 0){
		    // remember old value length
		    int old_vl = linked_node->tlv->value_length;

		    // set value_length, extra -1 to compensate for calculated value_length_size in update_parents
		    // for negative value_length, calculated value_length_size is 1
		    linked_node->tlv->value_length = 0 - linked_node->tlv->tag_value_size - 1;
		    // set session_id (both ans.1 node and linkednode since they can point to different nodes)
		    session_id = _session_id;
		    ...
		    // set current node(both asn.1 node and linked node) as inactive/unlinked
		    // session_id of ZERO(0) is not used for normal operation, sessions start from ONE(1)
		    session_id = 0;
...
void asn1::TLVNode::set_value(unsigned char* _data, unsigned int _length, bool _shallow){
	// set old
...
int asn1::decode_length(unsigned char* data, unsigned int data_length, TLVNode* tlv){
	if(data != NULL && data_length > 0 && tlv != NULL){
		if((*data & 0xFF) > 0x80){
			...
			//--data;
			//std::cout << "DEFINITE LONG: " << std::dec << tlv->tag_value << ":" <<  tlv->value_length_size << ":" << tlv->value_length << std::endl;
...
int asn1::decode(unsigned char* data, unsigned int data_length, asn1::ASN1Node* root_node, asn1::ASN1Node* root_defintion_node, asn1::ASN1Pool* _asn1_pool, uint64_t* _session_id){
    // used node count
    unsigned int used_nc = 0;
    // non CHOICE root node, make root_defintion_node a child of temporary parent node, process children of temporary parent node
	...
	int res = asn1::_decode(data, data_length, root_node, &root_asn1_node, _asn1_pool, _session_id, &used_nc);
	...
	return asn1::_decode(data, data_length, root_node, root_defintion_node, _asn1_pool, _session_id, &used_nc);
    }
...
int asn1::_decode(unsigned char* data, unsigned int data_length, ASN1Node* root_node, ASN1Node* root_defintion_node, ASN1Pool* _asn1_pool, uint64_t* _session_id, unsigned int* used_nc){
	// sanity checks
	...
	while(tmp_pos < data_length){
		...
		// get length
		if(decode_length(&data[tmp_pos], data_length - tmp_pos, tlv) != 0) return 300;
		...
		tlv->full_tlv_length = tlv->old_value_length + tlv->tag_value_size + (tlv->length_type == INDEFINITE ? 2 : 0);
			...
			for(unsigned int j = next_src_index; j<root_defintion_node->children.size(); j++) {
				...
				if(tmp_tag_value == CHOICE){
					...
					if(tmp_node_matched != NULL) {
					    ...
					    tmp_node_src->tlv->old_value_length = tlv->old_value_length;
					    tmp_node_src->tlv->value_length = tlv->value_length;
					    tmp_node_src->tlv->value_length_size = tlv->value_length_size;
				...
				// check if matched
				if(tmp_node_matched != NULL){
					// - SET is not ordered, start from first element every time (not in case of SET OF when unlimited_size is set)
					// - all other elements are ordered
					if(root_defintion_node->tlv->uni_tag_class == SET && !unlimited) next_src_index = 0; else next_src_index = j + 1;
					...
					if(root_defintion_node->tlv->uni_tag_class == CHOICE) {
					    // set choice node values
					    root_defintion_node->choice_selection = tmp_node_matched;
					    root_defintion_node->session_id = *_session_id;
					    root_defintion_node->linked_node = root_defintion_node;
					    // set relevant tlv values
					    root_defintion_node->tlv->tag_value_size = tlv->tag_value_size;
					    root_defintion_node->tlv->length_type = tlv->length_type;
					    root_defintion_node->tlv->old_value_length = tlv->old_value_length;
					    root_defintion_node->tlv->value_length = tlv->value_length;
					    root_defintion_node->tlv->value_length_size = tlv->value_length_size;
					    // change ber node linked node
		...
		if(tlv->complexity == CONSTRUCTED) res = _decode(tlv->value, tlv->value_length, new_node, tmp_node_matched, _asn1_pool, _session_id, used_nc);
		// error check
		if(res != 0) return res;
...
int asn1::_decode(unsigned char* data, unsigned int data_length, ASN1Node* root_node, ASN1Pool* _asn1_pool, unsigned int* used_nc){
	// sanity checks
	if(data == NULL) return 100;
	if(root_node == NULL) return 101;
	// **** ALLOW ZERO LENGH NODES ****
	...
	while(tmp_pos < data_length){
		...
		// get full tlv node length
		tlv->full_tlv_length = tlv->value_length_size +
				       tlv->value_length +
				       tlv->tag_value_size +
		...
		// if constructed, process more
		if(tlv->complexity == CONSTRUCTED) res = decode(tlv->value, tlv->value_length, new_node, _asn1_pool);
		// error check
		if(res != 0) return res;
...
void asn1::mem_transfer_deep(ASN1Node* root_node, unsigned char* dest_buffer, int dest_buffer_pos){
	if(root_node != NULL){
		...
		if(root_node->linked_node->tlv != NULL){
			...
			memcpy(&dest_buffer[dest_buffer_pos], tmp_tlv->value, tmp_tlv->value_length);
			tmp_tlv->value = &dest_buffer[dest_buffer_pos];
			dest_buffer_pos += tmp_tlv->value_length;
		    }

		    for(unsigned int i = 0; i<root_node->linked_node->children.size(); i++){
			mem_transfer_deep(root_node->linked_node->children[i], dest_buffer, dest_buffer_pos);
		    }
...
int asn1::encode(unsigned char* buffer, int buffer_length, ASN1Node* root_node, uint64_t _session_id, bool mem_switch){
			...
			if(linked_node->tlv != NULL){
				...
				if(linked_node->tlv->uni_tag_class != CHOICE){
					...
					// constructed
						...
						for(unsigned int i = 0; i<linked_node->children.size(); i++) {
							tmp_length = encode(buffer, buffer_length - total_length, linked_node->children[i], _session_id, mem_switch);
							buffer += tmp_length;
				...
				// CHOICE
					...
					if(linked_node->choice_selection != NULL){
					    tmp_length = encode(buffer, buffer_length - total_length, linked_node->choice_selection, _session_id, mem_switch);
					    total_length += tmp_length;
					    buffer += tmp_length;
					}
					/*
		...
		if(root_node->linked_node != NULL && root_node->session_id == _session_id){
			...
			if(linked_node->tlv != NULL){
				...
				// CHOICE
					...
					// children
					if(linked_node->choice_selection != NULL){
					    tmp_length = encode(buffer, buffer_length - total_length, linked_node->choice_selection, _session_id);
					    total_length += tmp_length;
					    buffer += tmp_length;
					}
					/*
...
uint32_t asn1::prepare_int(uint32_t val, unsigned int* req_bytes, int req_b_override){
    if(req_b_override == -1) *req_bytes = pmink_utils::bytes_required(val);
    else *req_bytes = req_b_override;
...
time_t asn1::unix_timestamp(const char* gen_time){
    ...
    int values[6];
    // YYYY
    int res = sscanf(gen_time, "%4u%2u%2u%2u%2u%2u", &values[0], &values[1], &values[2], &values[3], &values[4], &values[5]);
    if(res == 6){
...
int asn1::generalized_time(time_t ts, unsigned int msec, unsigned char* output, bool local_time){
    ...
    // .FFF
    if(msec > 0){
	c = sprintf((char*)--output, "%g", (tm_ts.tm_sec % 10) + ((double)msec / 1000));
	output += c;
	--bc += c;
    }
    // local or utc
