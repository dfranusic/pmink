...
void antlr::process_config_block(pANTLR3_BASE_TREE tmp_ast, config::ConfigItem* tmp_cfg, bool is_definition){
	int n2 = tmp_ast->children->count;
	pANTLR3_BASE_TREE tmp_ci;
	string tmp_str;
	//config::ConfigItem* sort_node = NULL;
	for(int j = 0; j<n2; j++){
			...
			if(tmp_ci->children != NULL){
				if(tmp_ci->children->count > 0){
					...
					if(tmp_str.substr(0, 3) == "/S/"){
					    ...
					    //std::cout << "SORT 1: " << sort_node << std::endl;
					    //std::cout << "PARENT: " << tmp_cfg->parent << std::endl;
		...
		// item
			...
			if(tmp_cfg->is_template &&
			   ...
			   tmp_cfg->children[0]->name == "...") {
			     //tmp_cfg->children.erase(tmp_cfg->children.begin());
			     tmp_cfg->children[0]->name = "";
		...
		// type
			...
			if(tmp_ci->children != NULL){
				if(tmp_ci->children->count > 0){
					...
					// script/module param
					if(tmp_cli->node_type == cli::CLI_SCRIPT || tmp_cli->node_type == cli::CLI_METHOD || tmp_cli->node_type == cli::CLI_CONST){
						if(tmp_ci->children != NULL){
							if(tmp_ci->children->count > 0){
								tmp_ci = (pANTLR3_BASE_TREE)tmp_ci->children->get(tmp_ci->children, 0);
								tmp_str = (char*)tmp_ci->toString(tmp_ci)->chars;
...
void antlr::process_config(pANTLR3_BASE_TREE p_ast_tree, config::ConfigItem* config_tree_res){
	...
	// node name/value
	// children
	    //std::cout << "START: " << p_ast_tree->toString(p_ast_tree)->chars << std::endl;
...
void antlr::build_config_def_tree(pANTLR3_BASE_TREE p_ast_tree, config::ConfigItem* config_tree, bool is_definition){
